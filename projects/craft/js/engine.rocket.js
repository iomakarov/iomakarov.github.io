...
var rocket = function( pars ) {
        ...
        with ( this.c2d ) {
            save();
                with ( this.tth ) {
                    ...
                    if(this.active)
                        socket.emit('onrocket',{rocket_id:this.rocket_id,x:this.pmmd.x,y:this.pmmd.y,
    ...
    this.ruling = function() {
    	with (Math) {
        	if (this.pmmd.u != this.ucel) {
        		this.tth.en[0].u = (this.pmmd.u - this.ucel);
        	}
    	}
    }
